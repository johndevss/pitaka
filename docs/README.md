...
| --- | --- |
| **[Architecture](architecture.md)** | High-level system architecture, Feature-First Clean Architecture layout, layer responsibilities, and Riverpod state management flow. |
| **[File Structure](file-structure.md)** | Complete directory tree and detailed file-by-file reference for `lib/` and `test/`. |
| **[Database & Schema](database-schema.md)** | Embedded SQLite setup, WAL mode, table DDLs, indexes, database triggers, and pre-aggregated analytics views. |
